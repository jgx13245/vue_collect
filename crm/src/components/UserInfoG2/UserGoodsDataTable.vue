<template>
  <div class="data-table">
    ...
    <el-table
      ...
      style="margin: 10px 0;"
      :data="list">
        <el-table-column v-for="key in keys"  :prop="key" :key="key" :label="key" align="center" v-if="key !== 'userid'"></el-table-column>
        <!-- <el-table-column
          align="center"
          fixed="right"
          label="操作"
...
export default {
  ...
  methods: {
    ...
    handleSaveExcel() {
      const tHeader = [['超市名称', '商品订单', '商品进价', '商品名称', '商品数量', '商品单价']]
      const tbody = this.list.map(({ 超市名称, 商品订单, 商品进价, 商品名称, 商品数量, 商品单价 }) => [超市名称, 商品订单, 商品进价, 商品名称, 商品数量, 商品单价])
